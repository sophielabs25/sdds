...
// Handle Update Property using POST/Redirect/GET pattern
...
// Handle Property Deletion using POST/Redirect/GET pattern
...
// Fetch Properties with Pagination (joined with sites)
$sql = "SELECT properties.*, sites.site_name,
        (
          SELECT COALESCE(SUM(total_beds * 
            CASE 
              WHEN bed_type IN ('Double', 'Queen', 'King') THEN 2 
              ELSE 1 
            END), 0)
          FROM rooms 
          WHERE property_id = properties.id
...
$total_stmt = $conn->prepare("SELECT COUNT(*) FROM properties JOIN sites ON properties.site_id = sites.id WHERE $filter_sql");
$total_stmt->execute($filters);
$total_properties = $total_stmt->fetchColumn();
...
<div class="container mt-5">
    ...
    <!-- Pagination: Include external pagination controls -->
    <?php 
      include 'pagination.php';
      renderPaginationControls($page, $total_pages, $limit, $_GET);
    ?>
</div>
...
<script>
    function showUpdateModal(property) {
        // property is passed as a JSON object
